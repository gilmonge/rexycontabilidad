<?php
	...
	**************************************************/
    ...
    /* Establece las urls de redireccion */
        ...
        $url_principal=$uri."admin/items.php";
        //$url_edit=$uri."admin/tipo-cuenta-editar.php";
    /* Establece las urls de redireccion */
    
    /* Incluye las librerias */
        require('class_libraries/items.php');
        $items = new items;
    /* Incluye las librerias */
    ...
    /* Decision de que metodo usar */
        switch($formaction){
            case "create_DB":
                $items->agregaDB($url_principal, $Quick_function, $_POST);
            break;
            case "edit_DB":
                $items->editarDB($url_principal, $Quick_function, $_POST);
            break;
            case "activate_DB":
                $items->activarDB($url_principal, $Quick_function, $_POST);
            break;
            case "deleted_DB":
                $items->borrarDB($url_principal, $Quick_function, $_POST);
            break;
            default:
                header('Location: '.$uri);
            break;
